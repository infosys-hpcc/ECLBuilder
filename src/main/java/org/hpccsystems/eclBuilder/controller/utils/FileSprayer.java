package org.hpccsystems.eclBuilder.controller.utils;
...
import org.hpccsystems.eclBuilder.Constants.EVENTS;
import org.hpccsystems.eclBuilder.controller.entity.SprayProgress;
import org.hpccsystems.eclBuilder.domain.FileMeta;
import org.hpccsystems.eclBuilder.entity.FileStructure;
import org.hpccsystems.eclBuilder.entity.SprayConfiguration;
...
public class FileSprayer implements Runnable {
	...
	private HPCCWsClient hpccConnection;
	 ...
	 */
	public FileSprayer(Desktop executionDesktop,
			EventListener<Event> eventListener,
			SprayConfiguration sprayConfiguration, HPCCWsClient hpccConnection,Platform platform) {
	...
	private void spray() {
		try {
			...
			logicalFile = logicalFile.startsWith(PREFIX) ? logicalFile : PREFIX
					+ logicalFile;
...
//			Platform platform = TreeController.getPlatformForCluster();
			...
			if (sprayConfiguration.isCSV()) {
				...
				sprayCSVFile(connector, file.getName(), temprayFileName,
						targetCluster, sprayConfiguration.getDelimitedOptions());

				postProgress(98, CONVERT_TO_THOR);
				spraySuccess = convertToTHOR(hpccConnection,
						sprayConfiguration.getStructure(), temprayFileName,
						logicalFile, false);

			} else if (sprayConfiguration.isFLAT()) {
				temprayFileName = logicalFile + "thor";

				spraySuccess = sprayFlatHPCCFile(connector, file.getName(),
						temprayFileName, sprayConfiguration.getRecordLength(),
						targetCluster, true);

				postProgress(98, CONVERT_TO_THOR);
				spraySuccess = addStrucutureToTHOR(hpccConnection,
						sprayConfiguration.getStructure(), logicalFile);
			} else if (sprayConfiguration.isXML()) {
				temprayFileName = logicalFile + "xml";

				sprayXMLFile(connector, file.getName(), temprayFileName,
						targetCluster);
	...
	private void setSprayStatus(String logicalFile, boolean spraySuccess) {
		...
		if (spraySuccess) {
			tempLogicalFile = tempLogicalFile.startsWith(PREFIX) ? tempLogicalFile
					.substring(1, tempLogicalFile.length()) : tempLogicalFile;
			tempLogicalFile = tempLogicalFile.contains("::") ? tempLogicalFile
					: ".::".concat(tempLogicalFile);
	...
	private void postProgress(int percent, String label) {
		schedule(new Event(EVENTS.ON_SPRAY_PROGRESS, null, new SprayProgress(
				percent, label)));
	}

	private void uploadToLandingzone(File file, HPCCWsClient connector)
			throws HPCCException {
		if (!connector.httpUploadFileToFirstHPCCLandingZone(file
				.getAbsolutePath())) {
			throw new HPCCException(Labels.getLabel("failedToTransferFile"));
		}
	}

	private String getTargetcluster(HPCCWsClient connector)
			throws HPCCException {
		try {
			...
			throw new HPCCException(
					Labels.getLabel("clusterNamesCouldNotBeFetched"), e);
		}
	}

	private void sprayXMLFile(HPCCWsClient connector, String fileName,
			String logicalFile, String targetCluster) throws HPCCException {
		String tempLogicalFile = logicalFile.startsWith(PREFIX) ? logicalFile
				: PREFIX + logicalFile;
		HPCCFileSprayClient fileSprayClient = connector.getFileSprayClient();

		try {
			ProgressResponse response = fileSprayClient.sprayLocalXML(fileName,
					tempLogicalFile, "", targetCluster, true,
					sprayConfiguration.getXmlFormat(),
					sprayConfiguration.getSprayRootTag(), 8192);
			...
			throw new HPCCException(
					"Unable to Spray file onto the target cluster", e);
		}
	}
	 ...
	 * 
	 */
	private void sprayCSVFile(HPCCWsClient connector, String fileName,
			String logicalFile, String targetCluster,
			DelimitedDataOptions options) throws HPCCException {
		...
		try {
			ProgressResponse response = fileSprayClient
					.sprayVariableLocalDropZone(options, fileName,
							tempLogicalFile, "", targetCluster, true,
			...
			throw new HPCCException(
					"Unable to Spray file onto a target cluster", e);
		}
	}

	private boolean addStrucutureToTHOR(HPCCWsClient hpccConnector,
			List<FileStructure> fields, String logicalFile)
			throws HPCCException {
		...
		ecl.append("ds := DATASET('").append(logicalFile).append("thor")
				.append("', struct,THOR); \n");
		ecl.append("OUTPUT( ds,,'").append(logicalFile).append(CLUSTER)
				.append(getTargetcluster(hpccConnector)).append(OVERWRITE);
	...
	private boolean deleteFile(String file) throws HPCCException {
		StringBuilder ecl = new StringBuilder();
		ecl.append("import STD; ").append("STD.File.DeleteLogicalFile('")
				.append(file).append("');");
	...
	private boolean convertToTHOR(HPCCWsClient hpccConnector,
			List<FileStructure> fields, String tempraryFileName,
			String logicalFile, boolean isXML) throws HPCCException {
		StringBuilder ecl = new StringBuilder();
		...
		if (isXML) {
			ecl.append(DS_DATASET).append(tempraryFileName)
					.append("',struct,XML('")
					.append(sprayConfiguration.getRootTag()).append("')); \n");
		} else {
			ecl.append(DS_DATASET).append(tempraryFileName)
					.append("',struct,CSV(HEADING(1))); \n");
		}
		ecl.append("OUTPUT( DS,,'").append(logicalFile).append(CLUSTER)
				.append(getTargetcluster(hpccConnector)).append(OVERWRITE);
	...
	private boolean executeECL(HPCCWsClient hpccConnection, String ecl)
			throws HPCCException {
		try {
			WorkunitInfo wu = new WorkunitInfo();
			wu.setECL(ecl);
			wu.setJobname("ECLBuilder_" + sprayConfiguration.getUserId()
					+ "_convert_to_thor");
			wu.setCluster(hpccConnection.getAvailableClusterGroups()[1]);
			wu.setResultLimit(100);
			wu.setMaxMonitorMillis(180000);
			LOGGER.debug("File spray Retries value is - {}",
					sprayConfiguration.getSprayRetryCount());
			String results = executeECLProgress(hpccConnection, wu,
					sprayConfiguration.getSprayRetryCount());

			LOGGER.debug("Results- {}", results);
			if (!StringUtils.isEmpty(results) && results.contains("Timed out")) {
				String workunit = results.substring(
						results.indexOf("<Message>") + 48,
						results.indexOf("</Message>"));
				LOGGER.debug("Results - {}", workunit);
				throw new HPCCException(Labels.getLabel("sprayTimeout1")
						+ hpccConnection.getHost()
						+ Labels.getLabel("sprayTimeout2") + workunit
						+ Labels.getLabel("sprayTimeout3") + workunit
						+ Labels.getLabel("sprayTimeout4"));
			}
		} catch (Exception e) {
	...
	private String executeECLProgress(HPCCWsClient hpccConnection,
			WorkunitInfo wu, int sprayRetryCount) throws  Exception{
		int count = sprayRetryCount;
		try {
			return hpccConnection.submitECLandGetResults(wu);
			...
			LOGGER.debug("Reaches maximum retries, file spraying failed - {}",
					e);
			throw new Exception(
					"Spray failed. HPCC cluster is busy. Please try again later");
		}
	}

	public List<FileMeta> getFileList(String scope, HPCCWsClient hpccConnection)
			throws HPCCException {
		...
		try {
			...
			DFULogicalFile[] files = hpccConnection.getWsDFUClient().getFiles(
					scope);
	...
	private FileMeta settingScope(String scope, FileMeta fileMeta,
			DFULogicalFile hpccLogicalFile) {
		if (scope.length() > 0) {
			fileMeta.setScope(scope + "::" + hpccLogicalFile.getName());
		} else {
			fileMeta.setScope(PREFIX + hpccLogicalFile.getName());
		}
		return fileMeta;
	}
	...
	private boolean handleSprayResponse(ProgressResponse sprayResponse)
			throws HPCCException {
		LOGGER.debug("Spray DFU Workunit - {} ; State - {}",
				sprayResponse.getWuid(), sprayResponse.getState());
		...
		if (fileSprayClient == null) {
			LOGGER.error(Constants.EXCEPTION, new HPCCException(
					ERROR_INITIALIZE_FILESPRAY_CLIENT));
		}
		try {
			ProgressRequest dfuprogressparams = new ProgressRequest();
			dfuprogressparams.setWuid(sprayResponse.getWuid());
			ProgressResponse progressResponse = fileSprayClient.getSoapProxy()
					.getDFUProgress(dfuprogressparams);
			if (progressResponse.getExceptions() != null) {
				LOGGER.error(Constants.EXCEPTION, new HPCCException(
						"Spray progress status fetch failed."));
			} else {
				if (!progressResponse.getState().equalsIgnoreCase(FAILED)) {
					// this should be in a dedicated thread.
					while (progressResponse.getPercentDone() < 100
							&& !progressResponse.getState().equalsIgnoreCase(
									FAILED)) {
						// progress bar value increasing based on percent done.
						postProgress(
								40 + (progressResponse.getPercentDone() * (58 / 100)),
								SPRAYING);
						progressResponse = fileSprayClient.getSoapProxy()
								.getDFUProgress(dfuprogressparams);
						Thread.sleep(100);
					}
			...
			throw new HPCCException(
					"Error occured while spraying the file in HPCC cluster", e);
		}
		return success;
	}
	 ...
	 */
	public boolean sprayFlatHPCCFile(HPCCWsClient connector, String fileName,
			String targetFileLabel, int recordSize, String targetCluster,
			boolean overwritesprayedfile) throws HPCCException {
		boolean success = true;

		try {
			HPCCFileSprayClient fileSprayClient = connector
					.getFileSprayClient();

			if (fileSprayClient != null) {
				success = handleSprayResponse(fileSprayClient
						.sprayFixedLocalDropZone(fileName, recordSize,
								targetFileLabel, "", targetCluster,
